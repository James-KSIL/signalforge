...
import { ForgeEvent, EventType, EventRole, EventContent } from './event.types';
...
function extractTags(event: ForgeEvent): EventTag[] {
  ...
  if (
    event.event_type === 'dispatch_seeded' ||
  ...
  if (
    contentText.includes('architecture') ||
  ...
  if (
    contentText.includes('normalize') ||
  ...
  // Regression keywords
  if (
    contentText.includes('regression') ||
 ...
 */
export function tagEvent(event: ForgeEvent): { event: ForgeEvent; tags: EventTag[] } {
 ...
 */
export function tagEvents(events: ForgeEvent[]): Map<string, EventTag[]> {
 ...
 */
export function filterByTag(events: ForgeEvent[], tags: EventTag[]): ForgeEvent[] {
 ...
 */
export function groupByTag(events: ForgeEvent[]): Map<EventTag, ForgeEvent[]> {
  const eventTags = tagEvents(events);
  const groups = new Map<EventTag, ForgeEvent[]>();

  const allTags: Set<EventTag> = new Set();
 ...
 */
export function tagStatistics(events: ForgeEvent[]): Record<EventTag, number> {
  const eventTags = tagEvents(events);
  const stats: Record<EventTag, number> = {
    'architecture': 0,
    'normalization': 0,
    'runtime-path': 0,
    'source-of-truth': 0,
