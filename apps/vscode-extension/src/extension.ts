...
import { SessionStatusReporter } from './services/sessionStatusReporter';
...
let extensionSharedDb: any = null;

function closeDbConnection(db: any, label: string): Promise<void> {
  return new Promise((resolve) => {
    ...
    try {
      const maybePromise = db.close((err: any) => {
        if (err) {
...
class SignalForgeTreeProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  getChildren(): Thenable<vscode.TreeItem[]> {
    ...
    const pinned = this.context.globalState.get('signalforge.pinnedProject') as any;
    ...
    const latest = this.context.globalState.get('signalforge.latestDispatch') as any;
    ...
    // active session
    const activeSession = this.context.globalState.get('signalforge.activeSession') as any;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function rehydrateActiveSessionFromDatabase() {
    const pinned = context.globalState.get('signalforge.pinnedProject') as any;
  ...
  rehydrateActiveSessionFromDatabase().catch((error: any) => {
    console.log('[SignalForge:db] failed to rehydrate active session', error?.message || String(error));
  });

  const sessionStatusReporter = new SessionStatusReporter(context);
  context.subscriptions.push(sessionStatusReporter);
  const sessionBootstrapService = new SessionBootstrapService(context, () => sharedDb);
  ...
  function resolveTargetWorkspace() {
    ...
    if (!targetRoot) throw new Error('No target workspace available.');
    return { targetRoot, pinned };
  }

  function resolveLatestDispatch() {
    const latest = context.globalState.get('signalforge.latestDispatch') as any;
    if (!latest || !latest.threadId) throw new Error('No latest dispatch recorded.');
    return latest;
  }

  async function recordLatestArtifactPath(key: string, filePath: string) {
    try {
      const artifacts = (context.globalState.get('signalforge.latestArtifacts') as any) || {};
      artifacts[key] = filePath;
      await context.globalState.update('signalforge.latestArtifacts', artifacts);
    } catch {}
  }

  function toCanonicalArtifactEvent(row: any, fallbackProjectId: string): any {
    let content: any = row && row.content;
    ...
    if (typeof content !== 'object') content = { summary: String(content) };
  ...
  function resolveWorkspaceContext() {
    ...
    const sessionId = (context.globalState.get('signalforge.activeSession') as any) || `session_${Date.now()}`;
  ...
  function severityLabel(severity: vscode.DiagnosticSeverity): 'error' | 'warning' | 'information' | 'hint' {
    switch (severity) {
  ...
  function categorizeDiagnostic(diagnostic: vscode.Diagnostic): 'import_resolution' | 'type_mismatch' | 'missing_symbols' | 'configuration_schema' | 'other' {
    const code = String(diagnostic.code ?? '').toLowerCase();
    const message = diagnostic.message.toLowerCase();
    const text = `${code} ${message}`;

    if (text.includes('cannot find module') || text.includes('module not found') || text.includes('module resolution')) return 'import_resolution';
    if (text.includes('cannot find name') || text.includes('is not defined') || text.includes('missing symbol')) return 'missing_symbols';
    if (text.includes('is not assignable') || text.includes('type') || code.startsWith('ts')) return 'type_mismatch';
    if (text.includes('config') || text.includes('schema') || text.includes('tsconfig') || text.includes('json')) return 'configuration_schema';

    return 'other';
  }

  function collectWorkspaceDiagnostics(targetRoot: string) {
  ...
  function groupDiagnostics(diagnostics: Array<{ category: string; file: string; severity: string; code: string; message: string; line: number; column: number; source: string }>) {
  ...
  function buildWorkspaceErrorsMarkdown(payload: {
    ...
    session_id: string;
    captured_at: string;
    summary: { total_errors: number; total_warnings: number; files_affected: number };
    diagnostics: Array<{ file: string; severity: string; code: string; message: string; line: number; column: number; source: string; category: string }>;
    architectural_context: { active_project: string; active_contract: string; pinned_project: string };
  }) {
    ...
    try {
      ...
      if (result.ok) {
        vscode.window.showInformationMessage(
          `SignalForge session ready for ${result.projectLabel || result.projectId || 'current workspace'}.`
        );
      } else {
  ...
  const pinProject = vscode.commands.registerCommand('signalforge.pinProject', async () => {
    const folders = vscode.workspace.workspaceFolders || [];
    let pick: vscode.WorkspaceFolder | undefined;
    if (folders.length === 0) {
  ...
  const resetExtensionState = vscode.commands.registerCommand('signalforge.resetExtensionState', async () => {
    ...
    const discoverSignalForgeKeys = (memento: vscode.Memento): string[] => {
      const discovered = new Set<string>();
      const internalMemento = memento as unknown as {
        _value?: Record<string, unknown>;
        _storage?: { _value?: Record<string, unknown> };
      };
      ...
      for (const key of directValues) {
        if (key.startsWith('signalforge.')) discovered.add(key);
      }
    ...
    const globalKeys = Array.from(new Set<string>([
      ...knownSignalForgeKeys,
      ...discoverSignalForgeKeys(context.globalState),
    ]));
    const workspaceKeys = Array.from(new Set<string>([
      ...knownSignalForgeKeys,
      ...discoverSignalForgeKeys(context.workspaceState),
    ]));

    await Promise.all([
      ...globalKeys.map((key: string) => context.globalState.update(key, undefined)),
      ...workspaceKeys.map((key: string) => context.workspaceState.update(key, undefined)),
    ]);
  ...
  const startSessionCmd = vscode.commands.registerCommand('signalforge.startSession', async () => {
    try {
      ...
      const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
      if (!targetRoot) { vscode.window.showErrorMessage('No workspace available to start session for.'); return; }
      // eslint-disable-next-line @typescript-eslint/no-var-requires
  ...
  const endSessionCmd = vscode.commands.registerCommand('signalforge.endSession', async () => {
    try {
      const active = context.globalState.get('signalforge.activeSession') as any;
      if (!active) { vscode.window.showInformationMessage('No active session in global state.'); return; }
      // eslint-disable-next-line @typescript-eslint/no-var-requires
        const openDatabase = require('@signalforge/core/dist/storage/db').openDatabase;
  ...
  const showActiveSessionCmd = vscode.commands.registerCommand('signalforge.showActiveSession', async () => {
    try {
      ...
      const folders = vscode.workspace.workspaceFolders || [];
      const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
      if (!targetRoot) { vscode.window.showInformationMessage('No workspace available.'); return; }
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { deriveProjectIdFromPath } = require('@signalforge/core/dist/projects/projectService');
      const { openDatabase } = require('@signalforge/core/dist/storage/db');
      ...
      if (!session) { vscode.window.showInformationMessage('No active session found.'); return; }
  ...
  const captureWorkspaceErrorsCmd = vscode.commands.registerCommand('signalforge.captureWorkspaceErrors', async () => {
    try {
      ...
      const payload = {
        type: 'workspace_errors_captured' as const,
  ...
  const materializeLatest = vscode.commands.registerCommand('signalforge.materializeLatestDispatch', async () => {
    ...
    try {
      ...
      const db = openDatabase();
      const projectId = pinned ? pinned.projectId : deriveProjectIdFromPath(targetRoot);
      const activeSession = context.globalState.get('signalforge.activeSession') as any || null;
      ...
      treeProvider.refresh();
      try { await vscode.commands.executeCommand('signalforge.refreshLatestDispatchFromStore'); } catch {}
    } catch (err: any) {
      vscode.window.showErrorMessage(String(err));
    }
  });
  ...
  const seedTest = vscode.commands.registerCommand('signalforge.seedTestDispatch', async () => {
    ...
    try {
      ...
      const pinned = context.globalState.get('signalforge.pinnedProject') as any;
      const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
      if (!targetRoot) throw new Error('No workspace available to seed test dispatch for.');
      const projectId = pinned ? pinned.projectId : deriveProjectIdFromPath(targetRoot);
      const now = new Date().toISOString();
      const events = [
        {
          ...
          session_id: context.globalState.get('signalforge.activeSession') as any || null,
  ...
  const inspectCmd = vscode.commands.registerCommand('signalforge.inspectDispatchEvents', async () => {
    let latest = context.globalState.get('signalforge.latestDispatch') as any;
  ...
  async function writeProjectFile(targetRoot: string, relDir: string, filename: string, content: string) {
  ...
  const generateAdr = vscode.commands.registerCommand('signalforge.generateAdrDraft', async () => {
    const latest = context.globalState.get('signalforge.latestDispatch') as any;
    if (!latest || !latest.threadId) { vscode.window.showErrorMessage('No latest dispatch to generate ADR for.'); return; }
    const pinned = context.globalState.get('signalforge.pinnedProject') as any;
    const folders = vscode.workspace.workspaceFolders || [];
    if (!pinned && folders.length > 1) { vscode.window.showErrorMessage('Multiple workspace folders present. Pin a project before generating ADR.'); return; }
    const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
    if (!targetRoot) { vscode.window.showErrorMessage('No target workspace available to write ADR into.'); return; }
    ...
    try {
      ...
      const projectId = pinned ? pinned.projectId : deriveProjectIdFromPath(targetRoot);
      const canonicalEvents = rows.map((row: any) => toCanonicalArtifactEvent(row, projectId));
  ...
  const generateSession = vscode.commands.registerCommand('signalforge.generateSessionSummary', async () => {
    const latest = context.globalState.get('signalforge.latestDispatch') as any;
    if (!latest || !latest.threadId) { vscode.window.showErrorMessage('No latest dispatch to summarize.'); return; }
    const pinned = context.globalState.get('signalforge.pinnedProject') as any;
    const folders = vscode.workspace.workspaceFolders || [];
    if (!pinned && folders.length > 1) { vscode.window.showErrorMessage('Multiple workspace folders present. Pin a project before generating session summary.'); return; }
    const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
    if (!targetRoot) { vscode.window.showErrorMessage('No target workspace available to write session summary into.'); return; }
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { getChatEventsByThread } = require('@signalforge/core/dist/repositories/chatEventRepository');
      // eslint-disable-next-line @typescript-eslint/no-var-requires
  ...
  const generateLinkedIn = vscode.commands.registerCommand('signalforge.generateLinkedInTopics', async () => {
    const latest = context.globalState.get('signalforge.latestDispatch') as any;
    if (!latest || !latest.threadId) { vscode.window.showErrorMessage('No latest dispatch to derive topics from.'); return; }
    const pinned = context.globalState.get('signalforge.pinnedProject') as any;
    const folders = vscode.workspace.workspaceFolders || [];
    if (!pinned && folders.length > 1) { vscode.window.showErrorMessage('Multiple workspace folders present. Pin a project before generating topics.'); return; }
    const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
    if (!targetRoot) { vscode.window.showErrorMessage('No target workspace available to write topics into.'); return; }
    try {
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { getChatEventsByThread } = require('@signalforge/core/dist/repositories/chatEventRepository');
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      ...
      const excerpts = events.map((e: any) => String(e.content).replace(/\s+/g, ' ').slice(0, 140));
      const topics: string[] = [];
      ...
      if (excerpts[3]) topics.push(`Testing & Validation: ${excerpts[3]}`);
      ...
      try {
        ...
        if (outcomes && outcomes.length > 0) {
          ...
          if (o.what_broke) topics.push(`Friction observed: ${String(o.what_broke).slice(0, 140)}`);
          if (o.next_step) topics.push(`Next steps: ${String(o.next_step).slice(0, 140)}`);
        }
      } catch (e) {
        // ignore
      }
  ...
  const logOutcomeCmd = vscode.commands.registerCommand('signalforge.logOutcome', async () => {
    try {
      ...
      const folders = vscode.workspace.workspaceFolders || [];
      const targetRoot = pinned ? pinned.workspaceRoot : (folders[0] && folders[0].uri.fsPath);
      if (!targetRoot) throw new Error('No workspace available to log outcome for.');
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { deriveProjectIdFromPath } = require('@signalforge/core/dist/projects/projectService');
      const projectId = pinned ? pinned.projectId : deriveProjectIdFromPath(targetRoot);
      const latest = context.globalState.get('signalforge.latestDispatch') as any;
  ...
  const openOutcomeCmd = vscode.commands.registerCommand('signalforge.openLatestOutcomeLog', async () => {
    try {
      ...
      if (!threadId) { vscode.window.showInformationMessage('No latest dispatch to inspect outcomes for.'); return; }
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { getOutcomesByDispatch } = require('@signalforge/core/dist/repositories/outcomeRepository');
      // eslint-disable-next-line @typescript-eslint/no-var-requires
        const openDatabase = require('@signalforge/core/dist/storage/db').openDatabase;
  ...
  const refreshCmd = vscode.commands.registerCommand('signalforge.refreshLatestDispatchFromStore', async () => {
    try {
      ...
      const db = sharedDb;
      const row: any = await getLatestDispatch(db);
  ...
  try { Promise.resolve(vscode.commands.executeCommand('signalforge.refreshLatestDispatchFromStore')).catch(() => {}); } catch {}
  ...
  const openLatestAdr = vscode.commands.registerCommand('signalforge.openLatestAdr', async () => {
    ...
    if (!p) { vscode.window.showInformationMessage('No ADR artifact recorded.'); return; }
    try { const doc = await vscode.workspace.openTextDocument(p); await vscode.window.showTextDocument(doc, { preview: false }); } catch (e) { vscode.window.showErrorMessage(String(e)); }
  });
  ...
  const openLatestSession = vscode.commands.registerCommand('signalforge.openLatestSessionSummary', async () => {
    ...
    if (!p) { vscode.window.showInformationMessage('No session summary recorded.'); return; }
    try { const doc = await vscode.workspace.openTextDocument(p); await vscode.window.showTextDocument(doc, { preview: false }); } catch (e) { vscode.window.showErrorMessage(String(e)); }
  });
  ...
  const openLatestTopics = vscode.commands.registerCommand('signalforge.openLatestLinkedInTopics', async () => {
    ...
    if (!p) { vscode.window.showInformationMessage('No LinkedIn topics recorded.'); return; }
    try { const doc = await vscode.workspace.openTextDocument(p); await vscode.window.showTextDocument(doc, { preview: false }); } catch (e) { vscode.window.showErrorMessage(String(e)); }
  });
  ...
  const openArtifactsFolder = vscode.commands.registerCommand('signalforge.openArtifactsFolder', async () => {
    try {
      ...
      if (!targetRoot) { vscode.window.showInformationMessage('No workspace available to open artifacts for.'); return; }
      const docsPath = path.join(targetRoot, 'docs');
      if (!fs.existsSync(docsPath)) { vscode.window.showInformationMessage('No docs folder found in project.'); return; }
      const uri = vscode.Uri.file(docsPath);
      try { await vscode.commands.executeCommand('revealFileInOS', uri); } catch (e) { try { await vscode.env.openExternal(uri); } catch { vscode.window.showErrorMessage('Failed to open artifacts folder.'); } }
    } catch (err: any) {
      vscode.window.showErrorMessage(String(err));
    }
  });
  ...
  const openLatestArtifacts = vscode.commands.registerCommand('signalforge.openLatestArtifacts', async () => {
    const artifacts = context.globalState.get('signalforge.latestArtifacts') as any || {};
    const items: {label: string, description: string, path: string}[] = [];
    if (artifacts.adr) items.push({ label: 'ADR', description: artifacts.adr, path: artifacts.adr });
    if (artifacts.session) items.push({ label: 'Session Summary', description: artifacts.session, path: artifacts.session });
    if (artifacts.topics) items.push({ label: 'LinkedIn Topics', description: artifacts.topics, path: artifacts.topics });
    if (items.length === 0) { vscode.window.showInformationMessage('No generated artifacts recorded.'); return; }
    ...
    if (!chosen) return;
    try { const doc = await vscode.workspace.openTextDocument(chosen.path); await vscode.window.showTextDocument(doc, { preview: false }); } catch (e) { vscode.window.showErrorMessage(String(e)); }
  });
