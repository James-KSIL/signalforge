...
export class SessionBootstrapService {
  ...
  private openCoreDatabase(callSite: string): any {
  ...
  async bootstrapCaptureSession(onProgress?: BootstrapProgressHandler): Promise<SessionBootstrapResult> {
    const emit = async (result: SessionBootstrapResult) => {
      if (onProgress) {
    ...
    if (!target.ok) {
      const blocked: SessionBootstrapResult = { ok: false, state: 'blocked', reason: target.reason };
      await emit(blocked);
      return blocked;
    }
    ...
    const projectResolved: SessionBootstrapResult = {
      ok: true,
    ...
    const pinnedState: SessionBootstrapResult = {
      ok: true,
    ...
    const dispatchSeeded: SessionBootstrapResult = {
      ok: true,
    ...
    const ready: SessionBootstrapResult = {
      ok: true,
      state: 'capture_ready',
      projectId: resolvedProject.projectId,
      projectLabel: resolvedProject.projectLabel,
  ...
  private resolveWorkspaceTarget(): { ok: true; workspaceRoot: string; workspaceLabel: string } | { ok: false; reason: string } {
    ...
    const isInfrastructureFolder = (workspaceRoot: string): boolean => {
    ...
    const pinned = this.context.globalState.get<any>('signalforge.pinnedProject');
  ...
  private async resolveProjectIdentity(workspaceRoot: string, workspaceLabel: string): Promise<{ ok: true; projectId: string; projectLabel: string } | { ok: false; reason: string }> {
    const pinned = this.context.globalState.get<any>('signalforge.pinnedProject');
  ...
  private async persistProjectPin(
    projectId: string,
    projectLabel: string,
    workspaceRoot: string
  ): Promise<{ ok: true } | { ok: false; reason: string }> {
    const current = this.context.globalState.get<any>('signalforge.pinnedProject');
  ...
  private async persistActiveProjectAuthority(
    projectId: string,
    projectLabel: string,
    workspaceRoot: string
  ): Promise<{ ok: true } | { ok: false; reason: string }> {
    const current = this.context.globalState.get<any>('signalforge.activeProject');
    ...
    if (
      current?.active_project_id === projectId &&
  ...
  private async seedDispatchState(
    projectId: string,
    projectLabel: string,
    workspaceRoot: string
    ...
    const ensureDispatchCandidateEvent = async (sessionId: string) => {
      const existingEvents = await getChatEventsByThread(db, sessionId);
      const alreadySeeded = Array.isArray(existingEvents)
        && existingEvents.some((event: any) => event && event.event_type === 'dispatch_candidate_created');
  ...
  private async emitBootstrapAuthority(projectId: string, projectLabel: string, workspaceRoot: string): Promise<void> {
    try {
      ...
      try {
        ...
        if (db.run) {
          db.run(
            `INSERT OR REPLACE INTO projects (project_id, name, git_root, workspace_uri, created_at, updated_at) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            [projectId, projectLabel, workspaceRoot, workspaceRoot, new Date().toISOString(), new Date().toISOString()],
            () => {}
          );
        }
      } catch (dbErr: any) {
