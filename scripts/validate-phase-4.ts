...
let tagEvent: any;
let extractPatterns: any;
let generateSignalIndex: any;
let generateInsights: any;
let generatePortfolioSignal: any;
let generateLinkedInTopics: any;

function loadCoreModules(): boolean {
  try {
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function assertField(obj: any, field: string, label: string): boolean {
...
function assertArray(obj: any, field: string, label: string): boolean {
...
function validateEventTagging(events: any[]): boolean {
  console.log('\nTEST 1: Event tagging — determinism and required tag vocabulary');
  try {
    // Determinism: tag same events twice, compare
    const tagged1 = events.map((e: any) => tagEvent(e));
    const tagged2 = events.map((e: any) => tagEvent(e));
    ...
    if (missingTags.length > 0) {
      console.error(
        `  ERROR: The following contract-required tags were never emitted: ${missingTags.join(', ')}`
      );
      console.error(
        '  Ensure sample events trigger all required tag rules, or that the tagger covers the full vocabulary.'
      );
      return false;
    }
...
function validatePatternStability(events: any[], outcomes: any[]): boolean {
  ...
  try {
    ...
    const ids1 = patterns1.patterns.map((p: any) => p.pattern_id).sort();
    const ids2 = patterns2.patterns.map((p: any) => p.pattern_id).sort();
...
function validateSignalIndex(events: any[], outcomes: any[], patterns: any): boolean {
  ...
  try {
    ...
    for (const field of metaFields) {
      if (!assertField(index, field, 'signal index')) return false;
    }

    // signals array
    if (!assertArray(index, 'signals', 'signal index')) return false;
...
function validateInsightsGeneration(events: any[], patterns: any, outcomes: any[]): boolean {
  ...
  try {
    ...
    for (const section of requiredSections) {
      if (!assertField(insights1, section, 'insights')) return false;
    ...
    if (!assertField(insights1, 'source_event_ids', 'insights')) return false;
...
function validatePortfolioSignal(events: any[], patterns: any, outcomes: any[]): boolean {
  ...
  try {
    ...
    for (const field of topLevelFields) {
      if (!assertField(portfolio, field, 'portfolio')) return false;
    }

    if (!assertArray(portfolio, 'engineering_signals', 'portfolio')) return false;
    if (!assertArray(portfolio, 'technical_capabilities_demonstrated', 'portfolio')) return false;
    if (!assertArray(portfolio, 'interview_narratives', 'portfolio')) return false;
    ...
    if (!assertField(portfolio, 'source_event_ids', 'portfolio')) return false;
...
function validateLinkedInTopics(events: any[], patterns: any): boolean {
  ...
  try {
    ...
    if (!assertField(topics, 'project_id', 'linkedin topics')) return false;
    if (!assertArray(topics, 'topics', 'linkedin topics')) return false;
    ...
    for (const topic of topics.topics) {
      ...
      for (const field of requiredTopicFields) {
        if (!assertField(topic, field, `topic "${topic.title || 'unknown'}"`)) return false;
      }
      ...
      if (!validCategories.includes(topic.category)) {
        console.error(
          `  ERROR: Topic "${topic.title}" has invalid category "${topic.category}". Must be one of: ${validCategories.join(', ')}`
        );
        return false;
      }
...
function validateDeterminism(events: any[], patterns: any, outcomes: any[]): boolean {
...
function validateCanonicalStream(events: any[], patterns: any): boolean {
  ...
  try {
    ...
    if (patterns.contexts instanceof Map) {
      ...
      for (const contexts of Object.values(patterns.contexts)) {
        if (Array.isArray(contexts) && (contexts as any[]).length > 0) {
          contextCount++;
        }
      }
    }
...
function validateProjectScoping(patterns: any): boolean {
  console.log('\nTEST 9: Project-scoped storage — no cross-project artifact mixing');
  try {
    if (!assertField(patterns, 'project_id', 'patterns result')) return false;
...
function validateNoExternalDependencies(): boolean {
...
// ─── Main runner ──────────────────────────────────────────────────────────────
export function runPhase4Validation(projectId: string): boolean {
  console.log('\n' + '='.repeat(60));
