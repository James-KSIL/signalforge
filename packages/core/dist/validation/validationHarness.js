...
exports.runValidationHarness = void 0;
const copilotValidationService_1 = require("./copilotValidationService");
function buildPayload(id, rawText, projectId = 'proj_signalforge', sessionId = 'sess_active_01') {
    return {
...
function mkPositiveText(fileA, fileB) {
    return [
        'What I changed',
...
function runValidationHarness() {
    ...
    for (const testCase of cases) {
        const result = (0, copilotValidationService_1.validateCopilotCandidate)(testCase.payload, testCase.context);
        const predicted = result.ok ? 'positive' : 'negative';
        if (testCase.expected === 'positive' && predicted === 'positive')
            tp += 1;
        else if (testCase.expected === 'negative' && predicted === 'negative')
            tn += 1;
...
exports.runValidationHarness = runValidationHarness;
