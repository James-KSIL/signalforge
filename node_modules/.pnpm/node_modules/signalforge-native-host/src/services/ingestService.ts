import type { NativeBridgeResponse, NativeInboundMessage } from '@signalforge/shared/dist/types/messages';
import { openDatabase, initSchema, getDefaultDbPath } from '@signalforge/core/dist/storage/db';
import { validateInbound } from '@signalforge/core/dist/validation/validateIncomingMessage';
import { ingestChatEvent } from '@signalforge/core/dist/ingestion/ingestChatEvent';
import { ingestArtifactBound } from '@signalforge/core/dist/ingestion/ingestArtifactBound';
import {
  insertCopilotCandidateStaging,
  getRecentCopilotCandidates,
  updateCopilotCandidateStatus,
  updateCopilotCandidateEvaluation,
} from '@signalforge/core/dist/repositories/copilotCandidateRepository';
import { insertCopilotExecutionArtifact } from '@signalforge/core/dist/repositories/copilotArtifactRepository';
import { insertChatGPTVerificationEvent } from '@signalforge/core/dist/repositories/verificationRepository';
import { insertOutcomeLog } from '@signalforge/core/dist/repositories/outcomeLogRepository';
import {
  CopilotCandidatePayload,
  passesContractGate,
  validateCopilotCandidate,
} from '@signalforge/core/dist/validation/copilotValidationService';
import { getActiveSessionByProject, getSessionById } from '@signalforge/core/dist/repositories/sessionRepository';
import crypto from 'crypto';
import fs from 'fs';
import os from 'os';
import path from 'path';

let nativeHostDb: any = null;
let nativeHostSchemaReady = false;

async function ensureNativeHostDb(): Promise<any> {
  if (!nativeHostDb) {
    nativeHostDb = openDatabase();
    nativeHostSchemaReady = false;
  }

  if (!nativeHostSchemaReady) {
    await initSchema(nativeHostDb);
...
export function closeNativeHostDatabase(): Promise<void> {
...
function accept(messageId: string): NativeBridgeResponse {
  return { type: 'ack', message_id: messageId, status: 'accepted' };
}

function reject(messageId: string, reason: string): NativeBridgeResponse {
  return { type: 'error', message_id: messageId, reason };
}

function warnAndAck(messageId: string, warning: string): NativeBridgeResponse {
  console.warn('[SignalForge] native-host ingest warning', { message_id: messageId, warning });
  return accept(messageId);
}

let cachedBootstrapAuthority: {
  project_id: string;
...
const workspaceRootHintsByProject = new Map<string, string>();
...
function resolveCandidateSource(rawSource: unknown): string {
...
function isTestRunnerProcess(): boolean {
  ...
  if (typeof process.env.JEST_WORKER_ID === 'string' && process.env.JEST_WORKER_ID.trim()) return true;
  if (typeof process.env.VITEST === 'string' && process.env.VITEST.trim()) return true;
  if (typeof process.env.MOCHA === 'string' && process.env.MOCHA.trim()) return true;

  return false;
}

function resolveCandidateContentHash(inbound: any): string | null {
...
const BOOTSTRAP_SIGNAL_FILE = process.env.SIGNALFORGE_BOOTSTRAP_SIGNAL_FILE?.trim()
  || path.join(path.dirname(getDefaultDbPath()), 'bootstrap-authority-event.json');

export function getBootstrapSignalFilePath(): string {
  return BOOTSTRAP_SIGNAL_FILE;
}

export function getBootstrapSignalDbPath(): string {
...
function normalizeWorkspaceRoot(value: unknown): string | null {
  ...
  if (lower.startsWith('proj_')) return null;
  if (!/[\\/]|^[a-z]:/i.test(normalized)) return null;
  return normalized;
}

function rememberWorkspaceRoot(projectId: string, workspaceRoot: unknown): void {
  const normalizedProjectId = String(projectId || '').trim();
  const normalizedWorkspaceRoot = normalizeWorkspaceRoot(workspaceRoot);
  if (!normalizedProjectId || !normalizedWorkspaceRoot) return;
  workspaceRootHintsByProject.set(normalizedProjectId, normalizedWorkspaceRoot);
}

async function getWorkspaceRootFromProjectRecord(db: any, projectId: string): Promise<string | undefined> {
  const normalizedProjectId = String(projectId || '').trim();
  ...
  return await new Promise((resolve) => {
    db.get(
      'SELECT workspace_uri, git_root FROM projects WHERE project_id = ? LIMIT 1',
      [normalizedProjectId],
      (err: any, row: { workspace_uri?: string | null; git_root?: string | null } | undefined) => {
        if (err || !row) {
          resolve(undefined);
          return;
        }
...
function getWorkspaceRootFromBootstrapSignal(projectId: string): string | undefined {
  try {
    if (!fs.existsSync(BOOTSTRAP_SIGNAL_FILE)) {
      return undefined;
    }

    const raw = fs.readFileSync(BOOTSTRAP_SIGNAL_FILE, 'utf8');
    if (!raw || !raw.trim()) {
      return undefined;
    }
...
async function resolveWorkspaceRootForProject(db: any, projectId: string): Promise<string | undefined> {
  const normalizedProjectId = String(projectId || '').trim();
...
function setBootstrapAuthority(msg: any): NativeBridgeResponse {
  try {
...
export function drainBootstrapAuthoritySignal(): NativeBridgeResponse | null {
  try {
    ...
    const parsed = JSON.parse(raw);
    if (
      parsed?.type !== 'bootstrap_authority' ||
    ...
    return normalized as NativeBridgeResponse;
  } catch {
...
async function getBootstrapAuthority(msg: any): Promise<NativeBridgeResponse> {
  ...
  try {
    ...
    try {
      ...
      const recentProject = await new Promise<{ project_id: string; name: string; updated_at: string; workspace_uri?: string | null; git_root?: string | null } | null>((resolve, reject) => {
        ...
        db.get(
          'SELECT project_id, name, updated_at, workspace_uri, git_root FROM projects ORDER BY updated_at DESC LIMIT 1',
          [],
          (err: any, row: { project_id: string; name: string; updated_at: string; workspace_uri?: string | null; git_root?: string | null }) => {
...
function resolveMessageId(parsed: NativeInboundMessage, payload: any): string {
...
async function resolveSessionAndDispatchContext(
  db: any,
  payload: CopilotCandidatePayload
): Promise<{ sessionId: string; dispatchId: string | null }> {
  let sessionId = payload.session_id;
  let dispatchId = payload.dispatch_id || null;

  if (!sessionId || sessionId === 'session_unbound') {
    const active = await getActiveSessionByProject(db, payload.project_id);
  ...
  if (!dispatchId) {
    const rows = await dbAll<{ dispatch_id: string | null }>(
      db,
      `SELECT dispatch_id FROM chat_events
       WHERE event_type = ? AND project_id = ?
       ORDER BY created_at DESC LIMIT 1`,
      ['dispatch_candidate_created', payload.project_id]
    );
...
async function resolveLatestDispatchForProject(
  db: any,
  projectId: string,
  sessionId: string
): Promise<string | null> {
...
async function emitValidationLifecycleEvent(
  db: any,
  eventType:
    | 'copilot_candidate_captured'
    | 'copilot_candidate_rejected'
    | 'copilot_implementation_validated'
  ...
  await ingestChatEvent(db as any, {
...
async function emitOutcomeForValidationResult(
  db: any,
  eventType: 'copilot_implementation_validated' | 'copilot_candidate_rejected',
  payload: {
    project_id: string;
    session_id: string;
    dispatch_id?: string | null;
  ...
  try {
    const outcomeId = `out_${crypto.randomBytes(8).toString('hex')}`;
    let outcomeStatus: 'success' | 'partial' | 'failed';
...
export async function handleInbound(raw: any): Promise<NativeBridgeResponse> {
  try {
    const parsed = raw as NativeInboundMessage;
    ...
    const v = validateInbound(parsed);
    if (!v.ok) return reject(messageId, v.error || 'invalid_payload');
    ...
    if (payload.type === 'copy_binding_requested') {
      ...
      try {
        ...
        const activeSession = await getActiveSessionByProject(db, projectId);
        ...
        try {
          const boundEvent = {
            ...
            selection_type: selectionType as 'manual' | 'canvas',
            source_url: sourceUrl,
            created_at: createdAt,
          };

          await ingestArtifactBound(db, boundEvent);
    ...
    if (payload.type === 'copilot_candidate_lookup_query') {
      ...
      const recent = await getRecentCopilotCandidates(lookupDb, projectId, sessionId || undefined, 60);
    ...
    if (payload.type === 'copilot_candidate_captured') {
      const inbound = payload as CopilotCandidatePayload;
      const resolved = await resolveSessionAndDispatchContext(db, inbound);
      const contentHash = resolveCandidateContentHash(inbound as any);
      const normalizedSource = resolveCandidateSource((inbound as any)?.source);
      const normalizedCandidate: CopilotCandidatePayload = {
        ...inbound,
      ...
      await insertCopilotCandidateStaging(db, {
      ...
      try {
        ...
        const validation = validateCopilotCandidate(normalizedCandidate, {
          workspaceRoot: resolvedWorkspaceRoot,
        });

        const contractGate = passesContractGate(normalizedCandidate, {
          workspaceRoot: resolvedWorkspaceRoot,
        });

        await updateCopilotCandidateEvaluation(db, normalizedCandidate.candidate_id, {
        ...
        if (!contractGate.gatePass) {
          ...
          await updateCopilotCandidateStatus(db, normalizedCandidate.candidate_id, 'rejected', gateFailureReason);
        ...
        await insertCopilotExecutionArtifact(db, {
        ...
        await updateCopilotCandidateStatus(db, normalizedCandidate.candidate_id, 'promoted', null);
        ...
        try {
          await updateCopilotCandidateStatus(db, normalizedCandidate.candidate_id, 'rejected', gateErrorReason);
        } catch (statusErr: any) {
    ...
    if (payload.type === 'artifact_bound') {
      await ingestArtifactBound(db as any, payload);
      return accept(messageId);
    }
    ...
    if (payload.type === 'chatgpt_turn_classified') {
      ...
      if (payload.classification === 'chatgpt_verification_response' && payload.role === 'assistant') {
        ...
        await insertChatGPTVerificationEvent(db, {
