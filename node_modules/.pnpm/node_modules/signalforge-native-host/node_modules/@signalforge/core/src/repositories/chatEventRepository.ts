...
export function insertChatEvent(db: any, row: Partial<ChatEventRow & { content?: any }>): Promise<void> {
  ...
  const contentObj = typeof row.content === 'string' ? (() => {
    try { return JSON.parse(row.content); } catch { return { summary: String(row.content || '') }; }
  })() : (row.content || { summary: '' });
...
export function getChatEventsByThread(db: any, chatThreadId: string): Promise<ChatEventRow[]> {
  const sql = `SELECT * FROM chat_events WHERE chat_thread_id = ? ORDER BY created_at ASC`;
  return new Promise((resolve, reject) => {
    if (typeof db.all === 'function') {
      db.all(sql, [chatThreadId], (err: any, rows: any[]) => {
