...
export interface SignalIndex {
  version: string; // Semver: changes when structure changes
  generated_at: string;
  project_id: string;
  time_range: {
    start: string;
 ...
 */
export function generateSignalIndex(
  projectId: string,
  events: ForgeEvent[],
  outcomes: any[] = [],
  ...
  const dispatches = new Set<string>(
    signals
      ...
      .filter((dispatchId): dispatchId is string => typeof dispatchId === 'string' && dispatchId.length > 0)
  );

  // Pattern statistics
  const patternsByCategory: Record<string, number> = {};
  const highestSeverity: Array<{ pattern_id: PatternId; name: string; severity?: string }> = [];

  for (const pattern of patterns) {
    const category = (pattern as any).type || 'unknown';
    ...
    if (pattern.type === 'failure-mode') {
      highestSeverity.push({
        ...
        name: pattern.name,
        severity: (pattern as any).severity,
      });
    }
  }
  ...
  for (const event of dispatchEvents) {
    if (!seen.has(`dispatch_${event.dispatch_id}`)) {
      signals.push({
        ...
        status: (event.content.status as 'success' | 'fail' | 'partial') || 'active',
      ...
      signals.push({
        ...
        status:
          event.event_type === 'session_ended'
            ? 'closed'
            : (event.content.status as 'success' | 'fail' | 'partial') || 'active',
        tags: extractTagsFromEvent(event),
        pattern_ids: findPatternsForEvent(event, patterns),
      });
  ...
  for (let i = 0; i < outcomes.length; i++) {
    ...
    if (!seen.has(`outcome_${outcomeId}`)) {
      signals.push({
        ...
        status: (outcome.status as 'success' | 'fail' | 'partial') || 'partial',
    ...
    if (pattern.type === 'architecture-decision') {
      ...
      if (!seen.has(decisionId)) {
        signals.push({
          ...
          type: 'decision',
          title: pattern.name,
          summary: (pattern as any).principle,
          timestamp: (pattern as any).lastDecision || fallbackTimestamp,
          project_id: projectId,
          status: 'success',
 ...
 */
function extractTagsFromEvent(event: ForgeEvent): string[] {
  ...
  if (content.includes('refactor')) tags.add('refactoring');
  if (content.includes('design')) tags.add('design');
  if (content.includes('test')) tags.add('testing');
  if (content.includes('performance')) tags.add('performance');
  if (content.includes('security')) tags.add('security');
  if (content.includes('migration')) tags.add('migration');
  if (content.includes('cleanup')) tags.add('cleanup');
 ...
 */
function extractTagsFromOutcome(outcome: any): string[] {
  const tags = new Set<string>();
  
  // Status
  if (outcome.status === 'fail') tags.add('failure');
  if (outcome.status === 'success') tags.add('success');
  if (outcome.status === 'partial') tags.add('partial');
  
  // Content
  const content = `${outcome.title || ''} ${outcome.what_changed || ''} ${outcome.what_broke || ''}`.toLowerCase();
  if (content.includes('refactor')) tags.add('refactoring');
  if (content.includes('broken')) tags.add('breakage');
  if (content.includes('test')) tags.add('testing');
  if (content.includes('design')) tags.add('design');
 ...
 */
function findPatternsForEvent(event: ForgeEvent, patterns: DetectedPattern[]): PatternId[] {
  const content = `${event.content.summary} ${event.content.details || ''}`.toLowerCase();
  return patterns
    .filter((p) => {
 ...
 */
function findPatternsForOutcome(outcome: any, patterns: DetectedPattern[]): PatternId[] {
  const content = `${outcome.title || ''} ${outcome.what_changed || ''} ${outcome.what_broke || ''}`.toLowerCase();
  return patterns
    .filter((p) => {
      const keywords = getPatternKeywords(p);
...
function getPatternKeywords(pattern: DetectedPattern): string[] {
  if (pattern.type === 'failure-mode') return (pattern as any).keywords || [];
  if (pattern.type === 'refactor-theme') return [pattern.name];
  if (pattern.type === 'architecture-decision') return [pattern.name, (pattern as any).principle];
  if (pattern.type === 'friction-point') return [(pattern as any).name];
  if (pattern.type === 'acceptance-criteria') return (pattern as any).criteria || [];
  return [];
}
 ...
 */
export function serializeSignalIndex(index: SignalIndex): string {
 ...
 */
export function deserializeSignalIndex(json: string): SignalIndex {
  return JSON.parse(json) as SignalIndex;
}
