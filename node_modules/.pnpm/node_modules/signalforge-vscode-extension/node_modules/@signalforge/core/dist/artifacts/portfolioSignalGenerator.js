...
exports.renderPortfolioSignalMarkdown = exports.generatePortfolioSignal = void 0;
/**
 * Generate portfolio signal from events, outcomes, and patterns
 */
function generatePortfolioSignal(projectId, events, patterns, outcomes = [], timeRangeStart, timeRangeEnd) {
...
exports.generatePortfolioSignal = generatePortfolioSignal;
/**
 * Extract portfolio signals from outcomes, patterns, and events
 */
function extractPortfolioSignals(events, outcomes, patterns) {
    const signals = [];
    ...
    for (const failureMode of failureModes) {
        signals.push({
            ...
            platform_relevance: 'Event stream integrity + failure diagnosis patterns are universal across systems',
            internal_tooling_relevance: 'Demonstrates ability to debug production issues in deterministic data pipelines',
            bullet_points: [
    ...
    for (const decision of archDecisions) {
        signals.push({
            ...
            internal_tooling_relevance: 'Demonstrates systems design thinking and constraint-aware architecture',
            bullet_points: [
    ...
    for (const friction of frictionPoints) {
        signals.push({
            ...
            platform_relevance: 'Workflow optimization and friction analysis are universally valuable',
            internal_tooling_relevance: 'Shows ability to identify and eliminate developer friction points',
            bullet_points: [
    ...
    const validationEvents = events.filter((e) => e.content.summary.toLowerCase().includes('validat') ||
    ...
    if (validationEvents.length > 0) {
        signals.push({
            ...
            platform_relevance: 'Deterministic systems are increasingly important for trustworthy AI and distributed systems',
            internal_tooling_relevance: 'Demonstrates test-driven architecture and quality-first mindset',
            bullet_points: [
 ...
 */
function identifyKeyAchievements(patterns, outcomes) {
    const achievements = [];
    ...
    if (cleanupPatterns.length > 0) {
        achievements.push(`${cleanupPatterns.length} proactive cleanup/refactoring initiatives completed`);
    }
 ...
 */
function identifyTechnicalCapabilities(patterns, events) {
    const capabilities = [];
 ...
 * Identify leadership indicators
 */
function identifyLeadershipIndicators(patterns, events) {
    const indicators = [];
    // Strategic thinking
    ...
    const documentsEvents = events.filter((e) => e.content.summary.toLowerCase().includes('document') ||
    ...
    // Quality focus
    const testingEvents = events.filter((e) => e.content.summary.toLowerCase().includes('test'));
 ...
 */
function identifySystemsThinking(patterns, events) {
    const indicators = [];
 ...
 * Create interview narratives
 */
function createInterviewNarratives(signals, patterns, outcomes) {
    const narratives = [];
    ...
    if (archDecisions.length > 0) {
        ...
        auditability and reproducibility—critical for a system designed to capture engineering signal.`,
            interview_question: 'Can you tell me about a time when you made a difficult technical decision with clear tradeoffs?',
            talking_points: [
    ...
    if (failureModes.length > 0) {
        ...
        explicit and centralized, debugging becomes systematic rather than guesswork.`,
            interview_question: 'Tell me about a bug you debugged that required deep system understanding.',
            talking_points: [
 ...
 */
function generatePortfolioSummary(signals, achievements, capabilities) {
    const lines = [];
    lines.push('SignalForge represents a full-stack engineering achievement:');
    lines.push('');
    lines.push('I built a deterministic artifact generation system that extracts engineering signal');
    lines.push('from canonical event streams. The system handles complex constraints (determinism,');
    lines.push('isolation, auditability) while providing deep insights into project patterns, failures,');
    ...
    lines.push('build solutions that scale from correctness at their foundation, not performance shortcuts.');
    return lines.join('\n');
}
/**
 ...
 */
function renderPortfolioSignalMarkdown(signal) {
    const lines = [];
    // Header
...
exports.renderPortfolioSignalMarkdown = renderPortfolioSignalMarkdown;
