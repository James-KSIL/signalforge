...
import { sendArtifactBound, sendBrowserEvent, sendCopilotCandidateLookup, sendCopyBindingRequested, getBridgeStatus, getBootstrapAuthority, ensureNativeBridgeConnected, onBootstrapAuthorityPush, } from './nativeBridge.js';
const bindingState = new BindingState();
void bindingState.whenReady();
let awaitingDispatchThreads = new Set();
const CHATGPT_URL_PATTERNS = ['https://chat.openai.com/*', 'https://chatgpt.com/*'];
function reinjectContentScriptForOpenTabs(reason) {
...
async function initializeBootstrapAuthority() {
    try {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'confirm_binding') {
        void bindingState.whenReady().then(async () => {
            ...
            const artifactEvent = bindingState.createBoundArtifactEvent(pending, selectedProject);
