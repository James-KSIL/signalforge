...
import { computePatternFrequencies } from '../patterns/patternExtractor';
...
export interface LinkedInTopic {
  ...
  is_public: boolean; // true = can post, false = internal only
 ...
 */
export function generateLinkedInTopics(
  projectId: string,
  events: ForgeEvent[],
  patterns: DetectedPattern[],
  sessionId?: string,
  multiSessionSpan?: { sessions: number; dateRange: string }
 ...
 */
function deriveEngineeringLessonTopics(
  patterns: DetectedPattern[],
  ...
  if (patterns.some((p) => p.type === 'architecture-decision')) {
    ...
    const hasDeterminism = archDecisions.some(
      (p) =>
        p.principle?.toLowerCase().includes('determini') ||
    ...
    if (hasDeterminism || patterns.length > 3) {
      topics.push({
        ...
        is_public: true,
        source_pattern_ids: archDecisions.map((decision) => decision.pattern_id).filter((id: string) => !!id),
      });
    }
  }
  ...
  if (failureModes.length > 0) {
    ...
    topics.push({
      ...
      is_public: true,
      source_pattern_ids: [topFailure.pattern_id].filter((id: string) => !!id),
    });
  }
  ...
  if (cleanupPatterns.length > 1) {
    topics.push({
      ...
      is_public: true,
      source_pattern_ids: cleanupPatterns.map((pattern) => pattern.pattern_id).filter((id: string) => !!id),
    });
  }
 ...
 */
function deriveBusinessImpactTopics(
  patterns: DetectedPattern[],
  frequencies: PatternFrequency[]
): LinkedInTopic[] {
  const topics: LinkedInTopic[] = [];
  ...
  if (
    patterns.some(
      (p) =>
        p.type === 'architecture-decision' &&
        ((p as any).principle?.toLowerCase().includes('canon') ||
          (p as any).principle?.toLowerCase().includes('source'))
    )
  ) {
    topics.push({
      ...
      is_public: true,
      source_pattern_ids: patterns.map((pattern) => pattern.pattern_id).filter((id: string) => !!id),
    });
  }
  ...
  if (failureModes.length > 0) {
    topics.push({
      ...
      is_public: true,
      source_pattern_ids: failureModes.map((pattern) => pattern.pattern_id).filter((id: string) => !!id),
    });
  }
 ...
 */
function deriveArchitecturalThinkingTopics(patterns: DetectedPattern[]): LinkedInTopic[] {
  const topics: LinkedInTopic[] = [];
  ...
  if (archDecisions.length > 0) {
    ...
    topics.push({
      ...
      is_public: true,
      source_pattern_ids: [decision.pattern_id].filter((id: string) => !!id),
    });
  }
 ...
 */
export function renderLinkedInTopicsMarkdown(collection: LinkedInTopicsCollection): string {
  const lines: string[] = [];

  lines.push('# LinkedIn Topic Suggestions');
  lines.push('');
  lines.push(
    `Generated: ${collection.generated_at.slice(0, 10)} | ${collection.sessions_analyzed} session(s)`
  );
 ...
 */
export function exportForSocialMedia(topic: LinkedInTopic): { text: string; hashtags: string } {
  const maxLength = 300; // Conservative length for most platforms
  const narrative = topic.narrative.replace(/\n\s+/g, ' ').trim();
  const truncated =
    narrative.length > maxLength ? narrative.slice(0, maxLength - 3) + '...' : narrative;
 ...
 */
export function mergeTopicsCollections(
  collections: LinkedInTopicsCollection[]
): LinkedInTopicsCollection {
  const allTopics = collections.flatMap((c) => c.topics);
  const uniqueTopics = Array.from(
    new Map(allTopics.map((t) => [t.title, t])).values()
  );
...
function getDeterministicTimestamp(events: ForgeEvent[]): string {
  if (events.length === 0) {
